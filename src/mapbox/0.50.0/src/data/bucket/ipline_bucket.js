 ...
 * @private
 */
class IPLineBucket implements Bucket {
    distance: number;
    e1: number;
    e2: number;
    e3: number;
    ...
    hasPattern: boolean;
    programConfigurations: ProgramConfigurationSet<IPLineStyleLayer>;
    segments: SegmentVector;
    uploaded: boolean;

    constructor(options: BucketParameters<IPLineStyleLayer>) {
    ...
    populate(features: Array<IndexedFeature>, options: PopulateParameters) {
        ...
        for (const {feature, index, sourceLayerIndex} of features) {
            ...
            if (this.hasPattern) {
                this.features.push(addPatternDependencies('ipline', this.layers, patternFeature, this.zoom, options));
            } else {
        ...
        // console.log("ipline_bucket.populate Over")
    }
    ...
    addFeature(feature: BucketFeature, geometry: Array<Array<Point>>, index: number, imagePositions: {[string]: ImagePosition}) {
        ...
        const miterLimit = layout.get('ipline-miter-limit');
        const roundLimit = layout.get('ipline-round-limit');
...
register('IPLineBucket', IPLineBucket, {omit: ['layers', 'features']});
export default IPLineBucket;
